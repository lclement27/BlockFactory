...
/**
 * Factory class that produces Stone blocks
...
public class StoneBlockFactory implements Factory {
	...
	public StoneBlockFactory() {
		this.bin = new Resource(0, ResourceType.STONE);
	}
	
	/**
	 * Method to take in a resource for the factory to store
	 * Takes in any object, but it should be a Resource of type Stone
	...
	public void takeResource(Object obj) throws IllegalArgumentException {
		if (obj instanceof Resource) {
			...
			if (resource.getType() != ResourceType.STONE) {
				throw new IllegalArgumentException("StoneBlockFactory can only take stone blocks");
			}
			
			else 
				this.bin.addWeight(resource.getWeight(), ResourceType.STONE);
		}
		
		else
			throw new IllegalArgumentException("Object not a Resource");
		
	}
	
	
	/**
	 * Method to produce a stone block
	 ...
	 * @return a new stone block
	 */
	@Override
	public Block produce() {
		if (bin.getWeight() < Const.STONE_BLOCK_WEIGHT) {
			Random r = new Random();
			double weight = Math.round((Math.abs(r.nextDouble()))*100.0)/10.0 ;	// creates a random weight for the new resource
			Resource resource = new Resource(weight, ResourceType.STONE);	// new resource to be taken in
			takeResource(resource);
			return produce();		// EXTENSION recursion
		}
		else {
			bin.subtractWeight(Const.STONE_BLOCK_WEIGHT, ResourceType.STONE);
			StoneBlock newBlock = new StoneBlock();
			return newBlock;
		}
	}
	
	/**
	 * Displays the amount of stone it has by the weight
	 */
	@Override
